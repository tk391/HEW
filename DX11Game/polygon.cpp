...
//オーバーロード
void SetPolygonPos(XMFLOAT2 pos) {
	g_posPolygon.x = pos.x;
	g_posPolygon.y = pos.y;
}
...
//オーバーロード
void SetPolygonSize(XMFLOAT2 size) {
	g_sizPolygon.x = size.x;
	g_sizPolygon.y = size.y;
}
...
//オーバーロード
void SetPolygonColor(XMFLOAT4 color)
{
	if (color.x != g_colPolygon.x || color.y != g_colPolygon.y || color.z != g_colPolygon.z|| color.w != g_colPolygon.w) {
		g_colPolygon.x = color.x;
		g_colPolygon.y = color.y;
		g_colPolygon.z = color.z;
		g_colPolygon.w =color.w;
		g_bInvalidate = true;
	}
}
