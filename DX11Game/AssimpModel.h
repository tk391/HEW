...
public:
	...
	void SetAnimIndex(int nAnimIndex);//アニメーション変更
	UINT GetAnimCount();//何のアニメーションか取得できる
	double GetAnimDuration(int nAnimIndex);//アニメーションの最大再生時間を取得できる
	void SetAnimTime(double dTime);//アニメーションのフレーム（時間）が指定できる
